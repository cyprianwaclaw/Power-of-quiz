    ...
    <div class="psa-input-text-template">
        <div class="form-group">
            <label v-if="label.length > 0" class="form-control-label d-block">
                <span>{{ label }}</span>
                <span class="text-danger text-md ml-1">{{
                    isRequired ? '*' : ''
                }}</span>
            </label>
            ...
            <span class="d-block invalid-feedback font-weight-500 text-sm">
                {{ errors }}
            </span>
        </div>
    </div>
...
export default {
    ...
    computed: {
        // True/false if the input is requiredtestname.value.length > 0
        isRequired() {
